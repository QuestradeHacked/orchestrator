...
internal class MockLogger<TCategoryName> : ILogger<TCategoryName>
{
    ...
    private IEnumerable<LogEntry> GetMessageEntriesForSpecificLogLevel(LogLevel logLevel) =>
        _entries.Where(x => x.Level == logLevel);

    private IEnumerable<LogEntry> GetMessagesStartingLogLevel(LogLevel logLevel) =>
        _entries.Where(x => x.Level >= logLevel);

    public int Count() => _entries.Count;

    public int CountExactLevel(LogLevel logLevel) =>
        GetMessageEntriesForSpecificLogLevel(logLevel).Count();

    public int CountFromLevel(LogLevel logLevel) =>
        GetMessagesStartingLogLevel(logLevel).Count();
